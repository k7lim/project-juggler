...
pj v1 facades over CASS (coding_agent_session_search) for session data. CASS has critical limitations:
- **Hours-long full reindex** to discover new project directories
- **FrankenSQLite instability** — 20+ upstream issues traced to the same root cause (#155, #172, #183, etc.)
- **No freshness guarantee** — `--watch` only monitors known dirs, never re-discovers
...
Replace CASS as the primary data source with direct filesystem parsing. CASS stays as an optional fallback via `SessionStore` Protocol (already implemented in `pj/session_store.py`).
...
- **CASS stays as option**: `SessionStore` Protocol makes swap mechanical
